...
- **v0.8 test-fixture override discipline**: when a new default gate is introduced (for example `summarize.min_transcript_after_clean`), timeout/retry integration fixtures that temporarily replace `CCMEM_CONFIG_PATH` must carry forward any required baseline overrides or they will silently bypass the code path under test.
...
- **v0.10 cache-friendly hook contract**: in file-based injection mode, `UserPromptSubmit` should keep `additionalContext` byte-stable (empty) and write dynamic retrieval payload into a `.ccmem/context-<session>.md` file, while `SessionStart` owns the fixed Read instruction. Retrieval regressions now need assertions on context file contents and metrics, not only injected hook output.
- **v0.11 migration idempotence**: schema-upgrade tests can replay a new migration after force-lowering `schema_meta.version` on a DB that already has the new tables. New SQL migrations should use `IF NOT EXISTS` / guarded inserts when replay against existing artifacts is plausible, or partial-upgrade repair tests will fail with duplicate-table errors.
- **v0.11 context-history surface**: file-based injection reviewability now depends on both `context_snapshots` and `context_write_log`, and diagnose coverage should exercise all three modes: summary, `--session`, and `--hash`.
- **v0.11 summarize length flow**: `summarize_pending` should parse LLM JSON with `skipTruncate`, then refine/truncate to `save.max_chars_per_memory` afterward. Truncating during parse masks whether the refiner path actually ran.
- **v0.11 embedding fallback**: OpenAI query embedding timeouts should fail fast via `embedding.openai_timeout_ms` and fall back to lexical retrieval inside `retrieveMemories()` instead of letting prompt-submit hook time out.
