...
- **[2026-06-04] README 里不要臆造官方 marketplace 的精确 slug**:本地文档能确认社区市场安装前缀是 `@claude-community`，但不能证明某个第三方插件进入官方市场后一定使用 `@claude-plugins-official/<name>` 这种对称形式。对官方市场安装说明要写成“用 `/plugin install` 并按 Claude Code UI 显示的条目安装”，避免把未验证的 slug 写进对外文档。
...
- **[2026-06-04] `stop`→daemon 的 bridge wait/running 路径也要单独锁住“同 seq 不重复入队”**:只在 `stop-hook-dispatch` 层证明 `(session_id, last_message_seq)` 去重还不够；真实产品路径里，旧 seq 可能已经被 daemon 取走并进入 bridge wait/running，这时后续同 seq stop 仍必须继续被 dedupe，不能在 e2e 路径里多排一条 queued summarize。最小回归是 `stop` 先产出 seq2 并让 bridge 卡在 running，再触发同 transcript 的第二次 stop，最后断言全程只有一条 seq2 任务并只产生一次 `summarize_pending_applied`。
...
- **[2026-06-03] `stop`→daemon 的 summarize e2e 也要锁住“queued retry 被新 seq 超车”这条闭环**:只在 daemon-loop 层证明 supersede 还不够；真实产品路径里 stop hook 还会继续入队新 seq、wake daemon、再跑 mainLoop。若 e2e 不锁，retry 任务可能在 stop 驱动下重新跑出 stale memory。最小回归是 `stop` 先产出 seq2 失败+queued retry，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
- **[2026-06-03] timeout-scheduled 的 `summarize_pending` queued retry 也必须走同一套 supersede 语义**:`mainLoop` 把 `claude -p timeout after ...` 也视为 retryable；因此不只 429/too-many-requests，timeout 生成的 queued retry 若在重跑前被更高 seq 超车，也必须在 worker 入口被标成 `superseded`。最小回归先锁 daemon task 级别的“seq2 timeout -> queued retry seq2 -> seq3 插队 -> retry seq2 superseded -> seq3 applied”闭环。
- **[2026-06-03] `stop`→daemon 的 timeout queued retry 也要锁住同样的 supersede 闭环**:真实产品路径里，stop hook 会先产出 timeout 失败后的 queued retry，再由后续 stop 产出更高 seq；若 e2e 不锁，这条 timeout retry 可能在 wake/mainLoop 路径里重跑出 stale memory。最小回归是 `stop` 先产出 seq2 timeout + queued retry，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
- **[2026-06-03] 默认 60s 的 `429 rate limit` fallback retry 也不能绕过 `summarize_pending` 的 supersede 语义**:`retry-after` 缺失时 loop 会回退到 60_000ms 的 queued retry；如果这段等待里已经来了更高 seq，旧 retry 也必须在 worker 入口被标成 `superseded`，不能把 stale 结果写进 memories。最小回归先锁 daemon task 级别的“seq2 429(no retry-after) -> queued retry seq2(60s) -> seq3 插队 -> retry seq2 superseded -> seq3 applied”闭环。
- **[2026-06-03] 默认 60s 的 `too many requests` fallback retry 也不能绕过 `summarize_pending` 的 supersede 语义**:`too many requests` 与 plain 429 一样都会回退到 60_000ms 的 queued retry；如果这段等待里已经来了更高 seq，旧 retry 也必须在 worker 入口被标成 `superseded`，不能把 stale 结果写进 memories。最小回归先锁 daemon task 级别的“seq2 too-many-requests -> queued retry seq2(60s) -> seq3 插队 -> retry seq2 superseded -> seq3 applied”闭环。
- **[2026-06-03] `stop`→daemon 的默认 60s `429 rate limit` fallback retry 也要锁住同样的 supersede 闭环**:真实产品路径里，stop hook 会先产出没有 `retry-after` 的 429 失败，再留下一个 60_000ms 的 queued retry；若后续 stop 已经把 transcript 推到更高 seq，这个旧 retry 也必须在 wake/mainLoop 路径里变成 `superseded`，不能重新写入 stale memory。最小回归是 `stop` 先产出 seq2 plain-429 + queued retry(60s)，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
- **[2026-06-03] `stop`→daemon 的默认 60s `too many requests` fallback retry 也要锁住同样的 supersede 闭环**:真实产品路径里，stop hook 会先产出没有 `retry-after` 的 `too many requests` 失败，再留下一个 60_000ms 的 queued retry；若后续 stop 已经把 transcript 推到更高 seq，这个旧 retry 也必须在 wake/mainLoop 路径里变成 `superseded`，不能重新写入 stale memory。最小回归是 `stop` 先产出 seq2 too-many-requests + queued retry(60s)，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
- **[2026-06-04] 显式 `retry-after: 2s` 的 summarize queued retry 也不能绕过 supersede 语义**:`retry-after` 解析成 2_000ms 只保证了调度时间正确，不代表旧 retry 可以安全重跑；如果这 2 秒等待里已经来了更高 seq，旧 retry 仍必须在 worker 入口被标成 `superseded`，不能把 stale 结果写进 memories。最小回归先锁 daemon task 级别的“seq2 `429 rate limit; retry-after: 2s` -> queued retry seq2(2s) -> seq3 插队 -> retry seq2 superseded -> seq3 applied”闭环。
- **[2026-06-04] `stop`→daemon 的显式 `retry-after: 2s` queued retry 也要锁住同样的 supersede 闭环**:真实产品路径里，stop hook 会先产出显式 `retry-after: 2s` 的 429 失败，再留下一个 2_000ms 的 queued retry；若后续 stop 已经把 transcript 推到更高 seq，这个旧 retry 也必须在 wake/mainLoop 路径里变成 `superseded`，不能重新写入 stale memory。最小回归是 `stop` 先产出 seq2 `429 rate limit; retry-after: 2s` + queued retry(2s)，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
- **[2026-06-04] 显式 `retry-after: 2m` 的 summarize queued retry 也不能绕过 supersede 语义**:`retry-after` 解析成 120_000ms 只保证了调度时间正确，不代表旧 retry 可以安全重跑；如果这 2 分钟等待里已经来了更高 seq，旧 retry 仍必须在 worker 入口被标成 `superseded`，不能把 stale 结果写进 memories。最小回归先锁 daemon task 级别的“seq2 `429 rate limit; retry-after: 2m` -> queued retry seq2(120000ms) -> seq3 插队 -> retry seq2 superseded -> seq3 applied”闭环。
- **[2026-06-04] `stop`→daemon 的显式 `retry-after: 2m` queued retry 也要锁住同样的 supersede 闭环**:真实产品路径里，stop hook 会先产出显式 `retry-after: 2m` 的 429 失败，再留下一个 120_000ms 的 queued retry；若后续 stop 已经把 transcript 推到更高 seq，这个旧 retry 也必须在 wake/mainLoop 路径里变成 `superseded`，不能重新写入 stale memory。最小回归是 `stop` 先产出 seq2 `429 rate limit; retry-after: 2m` + queued retry(120000ms)，再由新的 stop 产出 seq3，随后断言 queued seq2 retry 变成 `superseded`、旧结果不落库、seq3 正常 `summarize_pending_applied`。
...
- **[2026-06-04] timeout 类集成测试若要命中当前实现，必须用 `CCMEM_CONFIG_PATH` 注入 `llm.claude_p_timeout_per_task[taskType]`**:`claude-p.mjs` 现在优先读取 per-task timeout；旧的 `CCMEM_CLAUDE_P_TIMEOUT_MS=50` 夹具不会再稳定触发 `summarize_pending`/`weekly_synthesis` timeout。回归要么写临时 config 文件，要么显式覆盖对应 taskType，不能再依赖全局 env。
- **[2026-06-04] v0.3 命令面回归要同时锁 library path 和 CLI path**:`cmdList(..., { quarantined: true })`/`cmdResurrect(..., { quarantined:true|alerts:true })` 只测库层还不够；CLI 还承担 reason/evidence 文案、交互 prompt、summary 文案与 flag 路由。最小覆盖要成对出现：库层验证状态变更，CLI 层验证用户可见输出与 flag 生效。
- **[2026-06-04] `security_audit` 不能只靠 cron/admin 面回归，任务层也要单独锁住**:仅测 `scheduleCronTasks()` 和 `admin cron run security_audit` 只能证明任务被排出来，不能证明 `selectAuditCandidates()` 的 pool 去重、LLM verdict 应用、空批次 `security_audit_run` 审计与 lease 收敛仍正确。最小覆盖应至少有三条：候选去重/聚类、quarantine+alert 应用、0-candidate run。
