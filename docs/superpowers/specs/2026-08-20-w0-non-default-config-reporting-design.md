...
| 3 | 🔴 **spawn 真实 `./bin/ccmem admin diagnose`**（无 flag）断言 stdout 有那一行；再 spawn `--config` 断言全量列表 | **1 与 2** |
...
| CLI 可见性 | 同上文件 | **spawn 子进程**，给子进程自己的 `CCMEM_DATA_ROOT`（临时目录）+ 一份真 `config.json`。**父进程环境不动** |
...
⚠️ **spawn 时两条硬约束**（handoff Ⅳ.2 / Ⅳ.13）：
- **显式 `/usr/local/bin/node`** —— PATH 上是 nvm v22.13.1，**没有 fts5**，能力不同；
- **显式跑目标 checkout 的 `./bin/ccmem`** —— PATH 上的 `ccmem` 指向主仓库，
  A2 那类"绿了但测的不是本仓库"就是这么来的。
...
- 🔴 **W0 落地时 `security.tier3.block_user_explicit` 还在 `DEFAULT_CONFIG` 里**（W1 才删）
  ⇒ **W0 的测试不许拿这个真键当 unknown-key 样本**，否则 W1 删键那一步会把 W0 的测试弄红。
  **用合成 fixture 里的假键。**
