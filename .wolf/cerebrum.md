...
- **A timeout's percentile is computable from the censoring rate, even when the tail is unknowable**: with 43 of 131 real query-embed attempts cut off at `openai_timeout_ms: 800`, that 800ms bound sits at roughly the 67th percentile of the true latency distribution. The surviving success samples (322–799ms) are the distribution *conditioned on* finishing under the bound, so they cannot yield a p99 — but the censoring fraction alone locates the bound. Reach for this before proposing any sampling scheme: it often shows the constant is far worse-placed than "it's an upper bound" suggests.
- **A budget-bounded timeout must leave room for its own fallback path**: `prompt_submit` has a 2000ms `Promise.race` budget, and blowing it returns an *empty* context — strictly worse than the lexical fallback a timeout produces. So `embed_timeout + lexical-fallback cost (measured max 638ms) < 2000ms`, capping the timeout near 1360ms. Sizing the timeout off the latency distribution alone would overshoot into a regime that degrades the very case it was meant to protect.
- **A cache hit does not reset a consecutive-failure circuit breaker**: on 2026-08-02 the breaker opened after failures at 22:55:51, 22:57:29 and 23:08:57 — an intervening successful retrieval at 23:07:35 was a query-vector cache hit (`embedMs: 0`), never touched the provider, and so left the failure counter untouched. When reasoning about "consecutive" counters, only provider-touching calls count; success rows in metrics are not all equal.
...
- 2026-08-02: Do not use "live DB mtime/size unchanged before vs after" as proof that a read-only probe wrote nothing. On this machine the daemon and hooks write continuously, and a writer's auto-checkpoint moves `global.db` mtime while WAL size stays at its high-water mark — observed changing at 12:52:57 with no action from me at all. The valid proof is that the connection was opened `mode=ro` (the VFS blocks writes) plus a positive control: a `mode=ro` SELECT leaves mtime untouched. The handoff's discipline 7 states the mtime rule as if it were sound; it is not.
- 2026-08-02: When a later finding retracts a claim, do not record the retraction only in the finding that discovered it — go back and void the original wording where it still lives. `ps eww` was corrected inside Finding 12 while Finding 9 kept telling readers "this verification is not optional" for a full wave. Cross-referenced documents contradict themselves silently; the reader who lands on the older section has no signal that a newer one overrides it.
- 2026-08-03: A per-test opt-in safety wrapper (e.g. `withFakeLaunchctl(() => ...)`) is not enough when the danger is "reaches a real shared resource" — it has to be remembered by every future test author, and forgetting it fails silently (the test still passes while causing the real-world side effect). Code review on Task 4 caught this and required moving the override to module scope, executed once, so the whole file is safe regardless of what any test does or doesn't opt into. Prefer structural (module-scope, can't-forget-it) isolation over opt-in isolation whenever the failure mode of "forgot to opt in" is silent.
- 2026-08-03: Do not call `cmdAdminDaemon(db, { verb: 'restart'|'stop'|'start' })` in a test without also overriding `CCMEM_LAUNCHCTL_BIN`/`CCMEM_LAUNCHCTL_LOG` to a fake script. `CCMEM_LAUNCHAGENT_DIR` only relocates the plist *file*; `LAUNCHD_LABEL` ('com.ccmem.daemon') is fixed, and real `launchctl` keys jobs by domain+Label, not by which path they were bootstrapped from — so such a test collides with any real ccmem daemon installed on the machine (confirmed: it hijacked this dev machine's real launchd registration, verifiable via `launchctl print gui/<uid>/com.ccmem.daemon`). `tests/integration/admin-daemon-command.test.mjs` already has the fix pattern (`launchdLifecycleScript()` + a fake launchctl shell script that writes `daemon_lock` via `sqlite3`); reuse it, don't reinvent it.
- 2026-08-04: The interactive-alias trap is not limited to `cp` — `mv` is aliased interactively on this machine too, and it silently declined to overwrite `.wolf/buglog.json` (printing `overwrite ...? (y/n [n]) not overwritten`) while the surrounding `&&` chain kept running. Use `command mv` / `command cp` / an absolute path for any scripted write, and always verify the destination's content afterwards rather than trusting the exit code. Caught here only because the write was followed by a `jq length` check.
- 2026-08-04: Do not pipe `jq -r 'select(...)'` on whole objects into `wc -l` to count records — jq pretty-prints an object across many lines, so the count is inflated by a factor of however many fields the object has (counted 441 "cache hits" inside a bucket that held 109 rows total). Use `jq -c`, or project to a scalar. This is the same family as the project's "explain where a number came from before using it" discipline, on the high side rather than the zero side.
- 2026-08-04: Do not treat a measured claim in `docs/ccmem-v0.13-dogfood.md` as still true because it was verified once — some of them are statements about an ongoing process and expire. Finding 15 recorded "after the [circuit-breaker] fix: 6 failures, zero circuit opens", true when written at 12:20 on 2026-08-02; the breaker then opened 7 times that same evening between 23:10 and 23:20. Re-derive live-behaviour claims from current data before building on them, and date-stamp them when writing.
