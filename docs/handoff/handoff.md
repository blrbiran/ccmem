...
> 🆕🔴 **最新一轮（2026-08-09/10）：`openai_timeout_ms` 已从 800 抬到 1200 并合进 `main`，
...
1. **先跑 `git status` 和 `git log --oneline -20`，状态以你跑出来的为准。**
   ...
   **`raise-openai-timeout`（最新一轮，已合并未删）**，以及 `v0.13-spec` 等（**删分支/worktree 要先问**）。
...
5. **没有已知待修的产品 bug。** 已修五条，都在 `main`：读撕裂、写事务 517、review 两条收尾、
   `openDb()` pragma 次序、以及本轮的超时常数。**未修的是测试抖动 —— 动手前先读 Ⅹ。**
6. 🆕 **本轮撞出三条既有缺陷**，见 **Ⅺ.9**，其中 ① 已验证、③ 已解决：
...
9. 🆕 **下一轮的执行顺序（2026-08-10 交接，照此走即可；细节一律不在这里复述，去对应章节）：**
   ① **先修一处过时的文字**：`docs/superpowers/plans/2026-08-10-raise-openai-timeout.md` 的
      **Task 5 Step 3 仍写着「判据 1 和 3」，应为「1 和 2」**（判据 3 已裁决为不可操作化，见 **Ⅺ.14**），
      并核对该 Task 其余步骤有无别处引用判据 3。**prereg 本体零改动。** 很小，先做掉。
   ② **主任务：确定 v0.14 的优化方向。** 人类已明确目的 —— **不是修 paper，是定范围**；
      材料、方法与三条前置问题的现状见 **Ⅺ.15**（其中前两条已被人类裁决作废，别再问）。
   ③ **必须等测量窗口关闭（约 08-14/15）之后才做**：Task 5 读数（判据见 Ⅺ.12 与补遗）；
      以及 **A2/A1 硬编码路径清理 + `config.default.json` 值级一致性测试**
      —— 人类已裁决**合成一件**，见 **Ⅺ.13** 与本节第 6 条。
   **日常**：每天巡检中止判据 **1 与 2**（判据 2 在 `n<150` 时只作离群监视，不算 p99）。
   **建议 skills**：清单见 **Ⅷ**；本轮实测有效、下一轮点名推荐三个 ——
   `superpowers:brainstorming`（**开 v0.14 讨论之前必走**）、`superpowers:writing-plans`
   （定出方向之后再落计划）、`superpowers:verification-before-completion`（本轮两次自纠都靠它的纪律兜住）。
