...
test('stop hook wake supersedes a stale queued retry when a newer stop seq arrives before rerun', async () => {
  const db = openDb();
  const transcript = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-stale-retry.jsonl');
  const script = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-stale-retry.mjs');

  resetStopDaemonState(db);
  writeFileSync(script, "process.stderr.write('429 rate limit; retry-after: 1234ms');process.exit(29);");
  writeTranscript(transcript, 'remember my preference', 'ok');
  await handleStop(db, {
    session_id: 's-flow-bridge-stale-retry',
    transcript_path: transcript,
    cwd: process.cwd()
  });
  ...
  try {
    ...
    await Promise.race([
      mainLoop(db, () => stop, async (loopDb, task) => {
        await dispatchTask(loopDb, task);
        const payload = JSON.parse(task.payload ?? '{}');
        if (task.type !== 'summarize_pending' || payload.session_id !== 's-flow-bridge-stale-retry') {
          return;
        }

        if (payload.last_message_seq === 2) {
          writeFileSync(script, buildBridgeScriptSuccess('Bridge fresh retry result'));
          setBridgeCommand(script);
          return;
        }
  ...
  const tasks = db.prepare(
    `SELECT status, error_excerpt, attempts, json_extract(payload, '$.last_message_seq') AS last_message_seq
     FROM tasks
     WHERE type = 'summarize_pending'
       AND json_extract(payload, '$.session_id') = 's-flow-bridge-stale-retry'
     ORDER BY id ASC`
  ).all();
  const stale = db.prepare(
    `SELECT COUNT(*) AS n
     FROM memories
     WHERE source = 'auto_inferred' AND content = 'Bridge stale retry result'`
  ).get();
  const memory = getLatestAutoMemory(db);
  const audit = getLatestAudit(db, 'summarize_pending_superseded');
  const details = JSON.parse(audit.details);
  ...
  assert.equal(tasks[1].error_excerpt, null);
  assert.equal(stale.n, 0);
  assert.equal(memory.content, 'Bridge fresh retry result');
  assert.equal(memory.source, 'auto_inferred');
  assert.deepEqual(JSON.parse(memory.tags), ['stop-bridge']);
  assert.equal(audit.action, 'summarize_pending_superseded');
  assert.equal(details.session_id, 's-flow-bridge-stale-retry');
  assert.equal(details.last_message_seq, 2);
  assert.equal(applied.action, 'summarize_pending_applied');
  assert.equal(appliedDetails.session_id, 's-flow-bridge-stale-retry');
  assert.equal(appliedDetails.last_message_seq, 3);
  assert.equal(appliedDetails.inserted_count, 1);
...
test('stop hook wake supersedes a stale default-rate-limit retry when a newer stop seq arrives before rerun', async () => {
  const db = openDb();
  const transcript = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-default-stale-retry.jsonl');
  const script = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-default-stale-retry.mjs');

  resetStopDaemonState(db);
  writeFileSync(script, "process.stderr.write('429 rate limit');process.exit(29);");
  writeTranscript(transcript, 'remember my preference', 'ok');
  await handleStop(db, {
    session_id: 's-flow-bridge-rate-limit-default-stale-retry',
    transcript_path: transcript,
    cwd: process.cwd()
  });
  ...
  try {
    ...
    await Promise.race([
      mainLoop(db, () => stop, async (loopDb, task) => {
        await dispatchTask(loopDb, task);
        const payload = JSON.parse(task.payload ?? '{}');
        if (task.type !== 'summarize_pending' || payload.session_id !== 's-flow-bridge-rate-limit-default-stale-retry') {
          return;
        }

        if (payload.last_message_seq === 2) {
          writeFileSync(script, buildBridgeScriptSuccess('Bridge fresh default retry result'));
          setBridgeCommand(script);
          return;
        }
  ...
  const tasks = db.prepare(
    `SELECT status, error_excerpt, attempts, json_extract(payload, '$.last_message_seq') AS last_message_seq
     FROM tasks
     WHERE type = 'summarize_pending'
       AND json_extract(payload, '$.session_id') = 's-flow-bridge-rate-limit-default-stale-retry'
     ORDER BY id ASC`
  ).all();
  const stale = db.prepare(
    `SELECT COUNT(*) AS n
     FROM memories
     WHERE source = 'auto_inferred' AND content = 'Bridge stale default retry result'`
  ).get();
  const memory = getLatestAutoMemory(db);
  const audit = getLatestAudit(db, 'summarize_pending_superseded');
  const details = JSON.parse(audit.details);
  ...
  assert.equal(tasks[1].error_excerpt, null);
  assert.equal(stale.n, 0);
  assert.equal(memory.content, 'Bridge fresh default retry result');
  assert.equal(memory.source, 'auto_inferred');
  assert.deepEqual(JSON.parse(memory.tags), ['stop-bridge']);
  assert.equal(audit.action, 'summarize_pending_superseded');
  assert.equal(details.session_id, 's-flow-bridge-rate-limit-default-stale-retry');
  assert.equal(details.last_message_seq, 2);
  assert.equal(applied.action, 'summarize_pending_applied');
  assert.equal(appliedDetails.session_id, 's-flow-bridge-rate-limit-default-stale-retry');
  assert.equal(appliedDetails.last_message_seq, 3);
  assert.equal(appliedDetails.inserted_count, 1);
...
test('stop hook wake supersedes a stale second-based retry when a newer stop seq arrives before rerun', async () => {
  const db = openDb();
  const transcript = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-seconds-stale-retry.jsonl');
  const script = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-seconds-stale-retry.mjs');

  resetStopDaemonState(db);
  writeFileSync(script, "process.stderr.write('429 rate limit; retry-after: 2s');process.exit(29);");
  writeTranscript(transcript, 'remember my preference', 'ok');
  await handleStop(db, {
    session_id: 's-flow-bridge-rate-limit-seconds-stale-retry',
    transcript_path: transcript,
    cwd: process.cwd()
  });
  ...
  try {
    ...
    await Promise.race([
      mainLoop(db, () => stop, async (loopDb, task) => {
        await dispatchTask(loopDb, task);
        const payload = JSON.parse(task.payload ?? '{}');
        if (task.type !== 'summarize_pending' || payload.session_id !== 's-flow-bridge-rate-limit-seconds-stale-retry') {
          return;
        }

        if (payload.last_message_seq === 2) {
          writeFileSync(script, buildBridgeScriptSuccess('Bridge fresh second-based retry result'));
          setBridgeCommand(script);
          return;
        }
  ...
  const tasks = db.prepare(
    `SELECT status, error_excerpt, attempts, json_extract(payload, '$.last_message_seq') AS last_message_seq
     FROM tasks
     WHERE type = 'summarize_pending'
       AND json_extract(payload, '$.session_id') = 's-flow-bridge-rate-limit-seconds-stale-retry'
     ORDER BY id ASC`
  ).all();
  const stale = db.prepare(
    `SELECT COUNT(*) AS n
     FROM memories
     WHERE source = 'auto_inferred' AND content = 'Bridge stale second-based retry result'`
  ).get();
  const memory = getLatestAutoMemory(db);
  const audit = getLatestAudit(db, 'summarize_pending_superseded');
  const details = JSON.parse(audit.details);
  ...
  assert.equal(tasks[1].error_excerpt, null);
  assert.equal(stale.n, 0);
  assert.equal(memory.content, 'Bridge fresh second-based retry result');
  assert.equal(memory.source, 'auto_inferred');
  assert.deepEqual(JSON.parse(memory.tags), ['stop-bridge']);
  assert.equal(audit.action, 'summarize_pending_superseded');
  assert.equal(details.session_id, 's-flow-bridge-rate-limit-seconds-stale-retry');
  assert.equal(details.last_message_seq, 2);
  assert.equal(applied.action, 'summarize_pending_applied');
  assert.equal(appliedDetails.session_id, 's-flow-bridge-rate-limit-seconds-stale-retry');
  assert.equal(appliedDetails.last_message_seq, 3);
  assert.equal(appliedDetails.inserted_count, 1);
...
test('stop hook wake supersedes a stale minute-based retry when a newer stop seq arrives before rerun', async () => {
  const db = openDb();
  const transcript = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-minutes-stale-retry.jsonl');
  const script = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-rate-limit-minutes-stale-retry.mjs');

  resetStopDaemonState(db);
  writeFileSync(script, "process.stderr.write('429 rate limit; retry-after: 2m');process.exit(29);");
  writeTranscript(transcript, 'remember my preference', 'ok');
  await handleStop(db, {
    session_id: 's-flow-bridge-rate-limit-minutes-stale-retry',
    transcript_path: transcript,
    cwd: process.cwd()
  });
  ...
  try {
    ...
    await Promise.race([
      mainLoop(db, () => stop, async (loopDb, task) => {
        await dispatchTask(loopDb, task);
        const payload = JSON.parse(task.payload ?? '{}');
        if (task.type !== 'summarize_pending' || payload.session_id !== 's-flow-bridge-rate-limit-minutes-stale-retry') {
          return;
        }

        if (payload.last_message_seq === 2) {
          writeFileSync(script, buildBridgeScriptSuccess('Bridge fresh minute-based retry result'));
          setBridgeCommand(script);
          return;
        }
  ...
  const tasks = db.prepare(
    `SELECT status, error_excerpt, attempts, json_extract(payload, '$.last_message_seq') AS last_message_seq
     FROM tasks
     WHERE type = 'summarize_pending'
       AND json_extract(payload, '$.session_id') = 's-flow-bridge-rate-limit-minutes-stale-retry'
     ORDER BY id ASC`
  ).all();
  const stale = db.prepare(
    `SELECT COUNT(*) AS n
     FROM memories
     WHERE source = 'auto_inferred' AND content = 'Bridge stale minute-based retry result'`
  ).get();
  const memory = getLatestAutoMemory(db);
  const audit = getLatestAudit(db, 'summarize_pending_superseded');
  const details = JSON.parse(audit.details);
  ...
  assert.equal(tasks[1].error_excerpt, null);
  assert.equal(stale.n, 0);
  assert.equal(memory.content, 'Bridge fresh minute-based retry result');
  assert.equal(memory.source, 'auto_inferred');
  assert.deepEqual(JSON.parse(memory.tags), ['stop-bridge']);
  assert.equal(audit.action, 'summarize_pending_superseded');
  assert.equal(details.session_id, 's-flow-bridge-rate-limit-minutes-stale-retry');
  assert.equal(details.last_message_seq, 2);
  assert.equal(applied.action, 'summarize_pending_applied');
  assert.equal(appliedDetails.session_id, 's-flow-bridge-rate-limit-minutes-stale-retry');
  assert.equal(appliedDetails.last_message_seq, 3);
  assert.equal(appliedDetails.inserted_count, 1);
...
test('stop hook wake supersedes a stale too-many-requests retry when a newer stop seq arrives before rerun', async () => {
  const db = openDb();
  const transcript = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-too-many-requests-stale-retry.jsonl');
  const script = path.join(process.env.CCMEM_DATA_ROOT, 'stop-daemon-bridge-too-many-requests-stale-retry.mjs');

  resetStopDaemonState(db);
  writeFileSync(script, "process.stderr.write('too many requests');process.exit(29);");
  writeTranscript(transcript, 'remember my preference', 'ok');
  await handleStop(db, {
    session_id: 's-flow-bridge-too-many-requests-stale-retry',
    transcript_path: transcript,
    cwd: process.cwd()
  });
  ...
  try {
    ...
    await Promise.race([
      mainLoop(db, () => stop, async (loopDb, task) => {
        await dispatchTask(loopDb, task);
        const payload = JSON.parse(task.payload ?? '{}');
        if (task.type !== 'summarize_pending' || payload.session_id !== 's-flow-bridge-too-many-requests-stale-retry') {
          return;
        }

        if (payload.last_message_seq === 2) {
          writeFileSync(script, buildBridgeScriptSuccess('Bridge fresh too-many-requests retry result'));
          setBridgeCommand(script);
          return;
        }
  ...
  const tasks = db.prepare(
    `SELECT status, error_excerpt, attempts, json_extract(payload, '$.last_message_seq') AS last_message_seq
     FROM tasks
     WHERE type = 'summarize_pending'
       AND json_extract(payload, '$.session_id') = 's-flow-bridge-too-many-requests-stale-retry'
     ORDER BY id ASC`
  ).all();
  const stale = db.prepare(
    `SELECT COUNT(*) AS n
     FROM memories
     WHERE source = 'auto_inferred' AND content = 'Bridge stale too-many-requests retry result'`
  ).get();
  const memory = getLatestAutoMemory(db);
  const audit = getLatestAudit(db, 'summarize_pending_superseded');
  const details = JSON.parse(audit.details);
  ...
  assert.equal(tasks[1].error_excerpt, null);
  assert.equal(stale.n, 0);
  assert.equal(memory.content, 'Bridge fresh too-many-requests retry result');
  assert.equal(memory.source, 'auto_inferred');
  assert.deepEqual(JSON.parse(memory.tags), ['stop-bridge']);
  assert.equal(audit.action, 'summarize_pending_superseded');
  assert.equal(details.session_id, 's-flow-bridge-too-many-requests-stale-retry');
  assert.equal(details.last_message_seq, 2);
  assert.equal(applied.action, 'summarize_pending_applied');
  assert.equal(appliedDetails.session_id, 's-flow-bridge-too-many-requests-stale-retry');
  assert.equal(appliedDetails.last_message_seq, 3);
  assert.equal(appliedDetails.inserted_count, 1);
