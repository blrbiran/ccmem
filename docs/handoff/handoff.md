...
2. 🔴🔴 **第一件事是"别做什么"：有一个测量窗口正在跑，不要提前读它，也不要动判据。**
   ...
   **那份文件不许改，也不许另立判据。n < 150 次真实 embed 尝试时只许报"数据不足"。**
   实施计划与剩余步骤（Task 4 活体验证、Task 5 读数）见
   `docs/superpowers/plans/2026-08-10-raise-openai-timeout.md`；执行账本见
...
6. 🆕 **本轮撞出三条未修的既有缺陷（都只记账，未立项）**，见 **Ⅺ.9**：
   ① **至少 10 个集成测试硬编码主 checkout 的绝对路径** ⇒ 在 worktree 里读的不是被测代码
   （**推论：Ⅹ 里凡在 worktree 跑批、涉及这些文件的结论，可比性存疑 —— 未验证，值得单独查**）；
   ② **没有任何测试守 `config.default.json` 与 `DEFAULT_CONFIG` 的值级一致**（`v013-config-sync`
   只比对 `version` 与 key-path 形状）；③ 预登记里 "Wilson 95%" 未钉 z 值。
