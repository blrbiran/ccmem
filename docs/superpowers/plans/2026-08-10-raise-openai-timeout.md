...
不按"批次是否在跑"划，批次边界一律留至少 60s 收尾余量（Ⅰ 记过：边界差 6 秒就把 5.1% 读成 9.2%）。
...
### 🔴 巡检撞出的口径问题：`n` 到底数剔除前还是剔除后（**待人类裁决，Task 5 前必须定**）
...
08-11 那 66 条几乎全部落在本会话跑 W4 的那一整天里，而本文档上一节自己写着
**「本会话 2026-08-11 全天活跃，按 Ⅰ 的口径整段属"曝露不明"」**。

**问题**：预登记把 **`n ≥ 150 次真实 embed 尝试`** 写在「样本量」一节，
把 **「剔除本机跑批窗口」** 写在「分析口径」一节，**两者没有说清 `n` 是剔除前数还是剔除后数**。

- 按**剔除前**读：`n=114`，照 08-10→08-11 的速率很快就到 150。
- 按**剔除后**读：手上最多 48（且 08-10 当天也有会话活动，还要再打折），离 150 远得多。

⇒ **两种读法会给出完全不同的解封时点。** 这条**不由分析者临场选**（临场发明口径正是预登记要挡的事），
**记在这里等人类裁决**。它与中止判据 3 的字段问题同类：**预登记的口径在被执行时才暴露出没定义死。**

📌 **在裁决之前，上表的 `n` 一律按"原始"记，并显式标注它未经剔除** —— 不要让一个未定口径的数
悄悄变成解封依据。
