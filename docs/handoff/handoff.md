...
1. **The L2.5 probe must read `recent_injections`, never `memory_feedback`.** The latter's `outcome` is rewritten in place by feedback logic that runs *before* the probe, so reading it silently drops every turn where the legacy matcher fired. Guarded by invariants #130/#131. (#129 is the writeAudit/logAudit ban — the earlier reference here was wrong, and #129 itself was retargeted during final review because it could not fail as written.)
2. **The probe labels, it does not suppress.** Turns with no new injection still record rows. Discarding them was the original design and was explicitly overruled; that ruling stands and must not be reversed.
   *Corrected in final review (C3):* those rows were also being called the **negative control**, and they are not one. ccmem injects via `additionalContext`, which persists in the model's context for the rest of the session, so a re-probed earlier injection is a STALE injection, not an absent memory — and retrieval's LIKE fallback meant they fired 0 times in 281 live rows. They are now labelled `control: 'stale_injection'`, and the real noise floor is a separate cohort: K random **never-injected-this-session** memories scored against the same reply, labelled `control: 'random'`. `control` is the authoritative cohort field; `turn_aligned` is retained for continuity with rows written before it existed.
