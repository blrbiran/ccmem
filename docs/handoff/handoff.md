...
3. 🔴 **有一件事悬着，请先做**：探针的 `load_1m` / `cpus` 字段已上线、daemon 已于 `08-09 10:17:23` 重启并复核
   （进程比代码新），但**"真实样本确实带上了这两个字段"至今没有被观测到** —— 落盘的最后一行早于重启。
   **接手第一件事：读一行探针文件确认 `load_1m` 有值**（命令见 Ⅰ 末尾）。**有值就划掉这条；没值按 Ⅳ.5 排查。**
...
- 🔴 **未完成的一步：真实样本带上字段这件事尚未被观测到**（落盘最后一行早于重启）。**见快速接手第 3 条。**
