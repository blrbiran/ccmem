...
export function main(argv) {
  ...
  lines.push('  2. secretScan is not normalized, and it no longer carries a generic');
  lines.push('     "keyword + separator + value" pattern at all: credential_assignment was');
  lines.push('     removed after being measured over the real store (3,540 global rows) --');
  lines.push('     dead as shipped (two literal 0x08 bytes where \\b was meant, so it never');
  lines.push('     matched anything), then 6 hits / 6 false positives once repaired, then');
  lines.push('     1 hit / still a false positive once tightened. the four named patterns');
  lines.push('     remain and are the only secret detectors; the shape now uncovered is a');
  lines.push('     literal "password: <value>" written into a memory.');
