...
3. 🔴 **下一轮的第一件事（成本低，且卡着后面所有分析）：解开 Ⅶ 里 `15.3%` 与 `1/3` 的矛盾。**
   `config.mjs` 实测 `openai_timeout_ms: 800` 且"超时未重新定值"成立 ⇒ 探针自己给出 hook 失败率
   `P(ms>800)=15.3%`，而 Ⅶ 样本偏差第 3 条写的是"约 1/3"。**两者描述同一件事同一时期，差一倍以上。**
   若 1/3 为真 ⇒ **门槛裁决多余，直接抬超时**；若它是旧口径 ⇒ **就地作废**，它正在污染每一个基于 Ⅶ 的分析。
   化解方法 Ⅶ 里现成（探针 <800ms 段 vs hook 已知 105 条成功样本，`text_chars` 对 `prompt_chars` 校准）。
4. **探针数据仍在积累**，目标 n 按判据重推（见 Ⅶ，**不要照抄 300**）。取数方法见 Ⅰ 末尾。
...
19. **🆕 污染改为"记录已知事实"，不再靠推断（2026-08-09）。**
