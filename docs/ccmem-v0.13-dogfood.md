...
### Finding 15：`openai_timeout_ms: 800` 落在查询嵌入延迟分布的中间（P1 → **已取证，未修**）

**发现路径**：为回填 V4 的"生产计数"而按 Finding 12 修复时刻切分 `metrics.jsonl` 时撞见的。

**现象**（全部样本取自 Finding 12 修复之后，`hook=prompt_submit` 共 43 行）：

| `retrieval_path` | n | `retrieval_embed_error` | `retrieval_embed_ms` |
|---|---|---|---|
| `A`（走成语义） | 22 | — | 384–796ms（另 3 次 0ms = query 向量缓存命中） |
| `B-fail` | 14 | 全部 `Request timed out.` | **802–809ms** |
| `B-circuit` | 7 | — | — |

**成功样本的上界 796ms 与失败样本的下界 802ms 之间没有间隙。**
`openai_timeout_ms: 800` 不是一个"只有异常才会碰到"的上界，
它**落在真实查询嵌入延迟分布的中间** —— 约 1/3 的 prompt_submit 因此丢掉语义通道。
...
**与 Finding 14 的关系（顺带得到的正面证据）**：本条全部样本取自熔断阈值修复之后 ——
**7 次 `B-circuit` 全部早于该修复**，修复后 6 次失败、**零次熔断**，
其中两次连续失败仍未开闸。**Finding 14 的修复在生产中得到活证据。**
反过来说，修复前采集的任何 `B-fail`/`B-circuit` 比例都不能与本条数据混算。
