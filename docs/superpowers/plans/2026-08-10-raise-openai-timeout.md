...
1. **分位数方法从未钉死**，而中止判据 2 是一个**卡在 p99 上的阈值判定** ——
   这与补遗 1 处理的 `z` 是同一类缺口。脚本的处置是**同时算 nearest-rank 与线性插值并都打印**，
   两者落在 3000ms 两侧时报"不可判定"。**这是报告时的护栏，不是裁决。**
2. **补遗 4 把敏感性阈值钉成显示值 `38.7%`，原样继承了补遗 1 为 `40.3%` 修掉的那个舍入带。**
   实测 `59/125` 的未舍入下界是 **38.6631%** ⇒ **分歧带宽 0.0369pp**（比补遗 1 那条 0.0139pp 还宽）。
   上界若落在带内，按哪个值判**无人裁决**。
