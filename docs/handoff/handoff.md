...
> 🆕🔴🔴 **最新一轮（2026-08-11）：v0.14 的范围已定，其中 W4（daemon 成本计量）已实现并合进 `main`。**
...
9. 🆕 **下一轮的执行顺序（2026-08-11 交接，照此走即可；细节一律不在这里复述，去对应章节）：**
   ① **日常**：每天巡检中止判据 **1 与 2**（判据 2 在 `n<150` 时只作离群监视，不算 p99）。
      **判据 3 记为"不可操作化、未执行"，任何报告必须显式写出这一点，不得默认它通过。**
   ② **测量窗口关闭后才做**：Task 5 读数（口径见 Ⅺ.12 与补遗；ETA 见 Ⅻ.5）。
      **不许靠降 n 提前**；`n=150` 时区间若仍横跨 40.3%，预登记要求延到 `n ≥ 300`。
   ③ **主线：v0.14 剩下的三条工作流 W1 / W2 / W3**，范围与判据全在 `docs/ccmem-v0.14-spec.md`。
      **三条都必须等窗口关闭**（W3 改写入行为、W1/W2 保守起见同等）。见 **Ⅻ.1**。
   ④ **一件无人认领的小事**：`config.default.json` 与 `DEFAULT_CONFIG` 的**值级**一致性测试
      （现有 `v013-config-sync` 只比对 `version` 与 key-path 形状）。原裁决是"与路径清理合成一件"，
      而路径清理已单独做完 ⇒ **它现在独立且仍未做**，见本节第 6 条。
...
- 🆕 **2026-08-11 的六次跑批窗口**记在
  ...
  **不要只剔那六个 17 秒窗口** —— 按 Ⅰ 的规则整天属"曝露不明"。
