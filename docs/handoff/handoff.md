...
> ⚠️ **套件不是稳定绿的：112 次全量跑里 16% 会红**（详见 Ⅹ）。"540 pass / 0 fail" 只是单次跑的结果，不是基线。
> 探针仍在积累：**2026-08-08 干净快照 n=58**（目标 ~300）。
...
3. **唯一在等的事是探针数据积累**，n=58 / 目标 ~300。取数方法见 Ⅰ 末尾。**不要改探针代码**（样本要同源）。
4. **⚠️ 探针数字每轮都要重算，不要照抄：** 差值**不再是 0**（当前 `3/58 = 5.2%`，Wilson `[1.8%, 14.1%]`），
   **尾巴仍然肥** —— 58 个样本里 **5 个超出 ~1360ms 预算上限**，预算内的超时救不回它们。
   🔴 **取数前先剔除被你自己的重活污染的时段**（上一轮差点因此读出 12.2% 的假信号）。见 Ⅰ。
...
1. **差值不再是 0。** 轨迹：`0/22` → `2/55` → `3/58`。区间下界早已离开 0
   ⇒ **"抬超时能捞回一些"已有正证据**，只是量级仍不确定（当前 CI 宽达 `[1.8%, 14.1%]`）。
2. **尾巴肥。** 除了最早那个 `10003` 截尾，还有 `3635 / 5221 / 5285 / 5973`。
   ⇒ **58 个样本里 5 个（≈9%）超出 ~1360ms 的 prompt_submit 预算上限**，
...
- ⚠️ **套件基线不是"稳定绿"。** 单次跑是 540 tests；但 **112 次全量跑里 18/112 ≈ 16% 会红**，全部来自测试抖动，
...
- **成本提示**：纯取证轮 ~$19；实现轮 $60–195；bug-063 那轮 ~$160；**抖动复核这轮 ~$127**，
...
**整体 18/112 ≈ 16% 的全量跑会红。** 全部是测试抖动，**没有一条指向产品缺陷**。
...
- ⚠️ **"读撕裂"假说：既未确认，也未排除。** 把该窗口人为拉大到 15ms（busy wait）后**单跑 0/8 红**（基线同为 0/8）。
  **但这个实验是无效的** —— 读撕裂需要窗口内**有并发写**，而单跑时没有别的进程在动那一行（见下面"为什么单跑复现不了"）。
  **别把"0/8"读成排除，也别把假说当成已确认的根因照抄。**
